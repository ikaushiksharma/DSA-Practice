...
class Solution{
	ArrayList <Integer> boundary(Node node){
	    ...
	    res.add(node.data);
	    if(isLeaf(node)==true) return res;
	    addLeftBoundary(node,res);
	    addLeaves(node,res);
	    int n=res.size();
	    if(node.right!=null)addRightBoundary(node,res,n);
	    return res;
	}
	...
	void addRightBoundary(Node node,ArrayList<Integer> res,int idx){
	    Node curr=node.right;
	    while(curr!=null){
	        if(!isLeaf(curr))res.add(idx,curr.data);
